...
with st.sidebar:
    if os.path.exists("logo.png"):
        ...
        st.markdown(f'<center><img src="data:image/png;base64,{logo_b64}" width="150"></center>', unsafe_allow_html=True)
    else:
...
# --- MODULE 2: ENROLLMENT ---
    ...
    with st.form("reg"):
        ...
        if st.form_submit_button("REGISTER"):
            ...
            st.success(f"Registered: {n.upper()}")
...
# --- MODULE 3: SKILL PASSPORT ---
    ...
    with col1:
        ...
        skill = st.selectbox("Skill", ["First Aid", "Anatomy", "Vital Signs"])
        if st.button("STAMP"):
            new_s = {"ID": target_id, "Skill_Name": skill, "Status": "Certified", "Verified_By": "DR. CHAN SOKHOEURN"}
            st.session_state.skills_db = pd.concat([st.session_state.skills_db, pd.DataFrame([new_s])], ignore_index=True)
            save_to_excel(st.session_state.db, st.session_state.skills_db)
            st.success("Skill Stamped!")
    with col2: st.dataframe(st.session_state.skills_db, use_container_width=True)
...
# --- MODULE 4: CERTIFICATION ---
    ...
    if st.button("GENERATE LUXURY CERTIFICATE"):
        ...
        <div style="background:white; padding:40px; border:15px double #D4AF37; text-align:center; color:#002d5a;">
            ...
            <p>Completed the <b>{s['Level']}</b> Medical Roadmap.</p>
            <div style="display:flex; justify-content:space-around; margin-top:40px;">
                <div style="border-top:1px solid #333; width:180px;"><br>DR. CHAN SOKHOEURN</div>
                <img src="https://api.qrserver.com/v1/create-qr-code/?size=60x60&data={s['ID']}" width="60">
                <div style="border-top:1px solid #333; width:180px;"><br>SENIOR INSTRUCTOR</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
...
# --- MODULE 5: FINANCIAL HUB ---
    ...
    if st.button("💾 SAVE CHANGES"):
        ...
        save_to_excel(st.session_state.db, st.session_state.skills_db)
        st.success("Data secured in Excel!")
